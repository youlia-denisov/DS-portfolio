...
def _auto_tune_eps(X_scaled: np.ndarray) -> tuple[float, str]:
    ...
    Why sweep instead of k-distance elbow?
    The k-distance elbow requires visual inspection to find the bend.
    For automated selection we sweep a fixed grid and apply the two
    quantitative rules above — same idea, no human in the loop.
    """
    ...
    for eps in sorted(DBSCAN_EPS_CANDIDATES):
        ...
        # dominance: fraction of non-noise points in the largest cluster
        ...
        if noise_frac < DBSCAN_MAX_NOISE_FRAC and dominance < DBSCAN_MAX_DOMINANCE:
            ...
            break  # take the loosest eps that passes both filters
...
def _detect_dbscan(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    Resamples to hourly first — same logic as dbscan_analysis.py —
    because 15-min rows create micro-clusters with no behavioural meaning.
    """
    available_features = [c for c in FEATURE_COLS if c in df.columns]

    if len(available_features) < 3:
        # Not enough features: fall back to univariate on kWh only
        return _detect_iqr(df)[0].assign(method="DBSCAN_fallback_IQR"), {
            "note": "Insufficient feature columns; fell back to IQR"
        }

    # Resample to hourly (same aggregation as dbscan_analysis.py)
    df_work = df.copy()
    if "datetime" in df_work.columns:
        ...
        agg_dict["kWh"] = "sum"  # sum four 15-min slots into one hourly total
        df_hourly = (
    ...
    # Outlier score: distance to nearest non-noise point (approximated by
    # the 5th-nearest-neighbour distance in scaled space)
    if len(noise_rows) > 0:
...
def _detect_isolation_forest(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    Uses the same feature set as DBSCAN when available, otherwise falls
    back to kWh alone.

    Key concept: the 'contamination' parameter sets the expected fraction
    of anomalies. "auto" uses the original paper's threshold (score = -0.5).
    """
    available_features = [c for c in FEATURE_COLS if c in df.columns]
    ...
    df_result["outlier_score"] = -scores  # flip so higher = more anomalous
...
def _select_method(skewness: float, n_samples: int, rates: dict[str, float]) -> tuple[str, str]:
    ...
    Decision logic:
      - Highly skewed (>1.5): IQR — mean/std are distorted by the long tail
      - Moderate skew (0.5–1.5): IQR — still more robust than 3-sigma
      - Near-normal (<0.5): 3-sigma — classic method is appropriate here
      - Very small dataset (<200 readings): IQR — fewest assumptions
      - Sanity check: if 3-sigma flags >5% of data, distribution is too skewed
        for it to be reliable → fall back to IQR

    DBSCAN and Isolation Forest are shown for comparison but not auto-selected
    as the primary method because:
      - DBSCAN result depends heavily on the feature columns present
      - Isolation Forest adds randomness; result varies slightly each run
      Both are better used as cross-checks against the statistical methods.
    """
...
def run_outlier_pipeline(df: pd.DataFrame) -> OutlierResults:
    ...
    # Outlier rates for auto-selector
