...
def render_behavior_profile(df_clean):
    ...
    # Build the feature vector (this runs on every page load; cache it)
...
def _compute_features(df_clean: pd.DataFrame) -> pd.Series | None:
    ...
    We rename the kWh column if needed so the feature functions always receive
    a column literally called 'kWh', as the module expects.
    """
    try:
    ...
    # features.py expects columns: datetime, kWh
    df = df_clean.copy()
    ...
    # find the kWh column if it isn't already called 'kWh'
...
def _render_peak_behaviour(f: pd.Series):
    ...
    with col1:
        ...
        if hop is not None:
            # convert 0–23 to readable clock time
...
def _metric_with_bar(label, value, fmt, help_text, reference, lo, hi):
    ...
    # clamp to [lo, hi] for the visual bar
