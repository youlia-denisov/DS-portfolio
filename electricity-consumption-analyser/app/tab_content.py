...
# SIMPLE OVERVIEW (regular-user view)
# Combines key metrics + avg consumption heatmap in one friendly tab.
def render_simple_overview(daily_totals, date_col, daily_value_col, df_clean,
    ...
    st.markdown(
        ...
        "Darker red means that time slot is typically busier — you're running more appliances or heating/cooling."
    )
    ...
    if {"hour", "weekday"}.issubset(df_clean.columns):
        ...
        st.caption(
            "**How to read this:** each cell is one hour-of-day / day-of-week combination. "
            "A dark red cell (e.g. weekday evenings at 19:00) means you consistently use a lot of "
            "electricity then. Green cells are your quietest times. "
            "This can help you shift high-energy tasks (laundry, dishwasher) to cheaper off-peak hours."
        )
    else:
